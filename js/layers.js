    ...
    imageproc.operation = function(inputImage, outputImage) {
            ...
            if (currentShadeLayerOp == "dither" &&
                $("#dither-transparent").prop("checked")) {
                 ...
                 for (var i = 0; i < shadeLayer.data.length; i+=4) {
                    if (((shadeLayer.data[i] === 255) && (shadeLayer.data[i+1] === 255)) && (shadeLayer.data[i+2] === 255)) { // Current pixel is white
        ...
        if (currentOutlineLayerOp != "no-op") {
            ...
            if (currentOutlineLayerOp == "sobel" &&
                ...
                /**
                 * TODO: You need to show the shade layer (shadeLayer) for the non-edge pixels (transparent)
                 */
                 ...
                 for (var i = 0; i < outlineLayer.data.length; i+=4) {
                    if ($("#sobel-flip").prop("checked")) { // Flip
                        // Edge --> Black; Non-edge --> White (255)
                        if (outlineLayer.data[i] === 255) { // Current pixel is a non-edge pixel
                        ...
                        // Edge --> White (255); Non-edge --> Black
                        if (outlineLayer.data[i] === 0) { // Current pixel is a non-edge pixel
                            // Replace the pixel by the shade layer pixel
                            outlineLayer.data[i] = shadeLayer.data[i];
                            outlineLayer.data[i+1] = shadeLayer.data[i+1];
                            outlineLayer.data[i+2] = shadeLayer.data[i+2];
